/** Forms.app nested webhook payload parser (server-safe, no mock data). */
...
function flattenPayloadFields(payload: unknown, prefix = ''): Record<string, string> {
...
/** Parse Forms.app payload — nested structure first, flat fallback for legacy/simulate payloads. */
...
export function applyFieldMapping(
  ...
  fieldMapping: Array<{ externalField: string; appField: string }>,
  ...
  for (const { externalField, appField } of fieldMapping) {
    ...
    if (!val) {
      ...
      for (const [key, value] of Object.entries(rawFields)) {
        if (normalizeFieldKey(key) === target && value.trim()) {
          val = value.trim();
          break;
        }
      }
