...
import { registerFormConnection, type StoredFormConnection } from '../../server/external-forms/store';
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  res.status(404).json({ error: `Unknown external-forms action: ${action || '(missing)'}` });
}
