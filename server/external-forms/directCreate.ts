...
export async function directCreateFromFormsWebhook(params: {
  ...
  if (!conn) {
    return { status: 404, body: { ok: false, error: 'Connection not found' } };
  }
  if (!conn.isActive) {
    return { status: 403, body: { ok: false, error: 'Connection inactive' } };
  }
  if (conn.secretKey && params.secret !== conn.secretKey) {
    return { status: 401, body: { ok: false, error: 'Invalid secret key' } };
  }
